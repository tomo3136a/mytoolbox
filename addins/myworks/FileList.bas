...
'     8: ベースパス化
'     9: パスセグメント化
Sub PathProc(ra As Range, mode As Integer)
    ScreenUpdateOff
    '
    Select Case mode
    Case 1: Call GetFileList(ra)
    Case 2: Call GetFolderPath(ra, GetBookBool("path.1"))
    Case 3: Call GetFilePath(ra, GetBookBool("path.1"))
    ...
    For Each obj In fso.GetFolder(path).SubFolders
        If GetBookBool("path.4") Or Not re.Test(obj.name) Then
            ...
            If GetBookBool("path.3") Then
                ...
                ElseIf GetBookBool("path.2") Then
